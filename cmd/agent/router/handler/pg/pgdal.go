...
func (db *DbContent) GenerateQuerySQL(tableName string, queryFields []string, filterFields []dbbase.FilterField, condition map[string]string) (*dbbase.DbSQL, error) {
	...
	sqlTableName := fmt.Sprintf("%s.%s", db.SchemaName, tableName)
	...
	if filterFields != nil && len(filterFields) > 0 {
		...
		sqlSelect = fmt.Sprintf("SELECT %s FROM %s WHERE %s", sqlQueryField, sqlTableName, sqlFilter)
	} else {
		sqlSelect = fmt.Sprintf("SELECT %s FROM %s", sqlQueryField, sqlTableName)
	}
...
func (db *DbContent) GenerateInsertSQL(tableName string, insertField []dbbase.FieldCell) (*dbbase.DbSQL, error) {
	...
	sqlInsertStr := fmt.Sprintf(`INSERT INTO %s (%s) VALUES (%s)`, sqlTableName, sqlInsertField, sqlInsertValue)
...
func (db *DbContent) GenerateUpdateSQL(tableName string, updateField []dbbase.FieldCell, filterFields []dbbase.FilterField) (*dbbase.DbSQL, error) {
	...
	if filterFields != nil && len(filterFields) > 0 {
		...
		sqlUpdateStr = fmt.Sprintf("UPDATE %s SET %s WHERE %s", sqlTableName, sqlField, sqlFilter)
	} else {
		sqlUpdateStr = fmt.Sprintf("UPDATE %s SET %s", sqlTableName, sqlField)
	}
...
func (db *DbContent) GenerateDeleteSQL(tableName string, filterFields []dbbase.FilterField) (*dbbase.DbSQL, error) {
	...
	if filterFields != nil && len(filterFields) > 0 {
		...
		sqlDeleteStr = fmt.Sprintf("DELETE FROM %s WHERE %s", sqlTableName, sqlFilter)
	} else {
		sqlDeleteStr = fmt.Sprintf("DELETE FROM %s", sqlTableName)
	}
...
func (db *DbContent) RunQuery(sql *dbbase.DbSQL) error {
	...
	rows, err := db.DbConn.Query(sql.SqlStr)
	if err != nil {
		return err
	}
	...
	for _, value := range db.AllFieldsOfTable[sql.TableName] {
	...
	for rows.Next() {
		...
		// 读取一行中的所有数据
		rawResult := make([]interface{}, len(*column))
