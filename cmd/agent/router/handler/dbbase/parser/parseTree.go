...
func (p *Parser) infixToPostfix(tokens []*Token) (*tokenQueue, error) {
	...
	for len(tokens) > 0 {
		...
		if _, ok := p.Functions[token.stringValue]; ok {
			...
			if token.stringValue == "or" || token.stringValue == "and" {
				if hasGroup {
					token.stringValue += "_"
					token.Value = token.Value.(string) + "_"
				}
			}
			stack.push(token)
			wasLiteral = false
...
func (p *Parser) postfixToTree(queue *tokenQueue) (*ParseNode, error) {
	...
	for !queue.empty() {
		...
		if _, ok := p.Functions[stack.peek().Token.stringValue]; ok {
			...
			// pop off function parameters,这里考虑一下方法参数数可变的情况，比如substring可传2或3个参数。
			...
			//}
			stack.push(node)
		} else if _, ok := p.Operators[strings.Trim(stack.peek().Token.stringValue, "_")]; ok {
			// if the top of the stack is an operator
			node, err := stack.pop()
			if err != nil {
				return nil, err
			}
			o := p.Operators[strings.Trim(node.Token.stringValue, "_")]
